...
The FSA groupings reflect deciles of cumulative \covid cases, excluding cases among residents of long-term are homes,
between January 2020--March 28. 2021.
% SM: if using ICES dashboad FSA-decile; the date is Jan 2020 to Jan 16, 2021 if using Brown et al FSA-decile.
% and for this sentence, cite Brown et al or the ICES Dashboard.
% TODO: double check this period.
...
\begin{figure}
  ...
  \caption{Ontario forward sorting areas (FSAs, $N=513$),
    ...
    % TODO: double check this period. % SM: see above
...
We obtained the final output contact matrices $C_{aa'y}$ for Canada from \cite{Prem2017},
...
% SM: here, can we cite some of Amir's papers as a reference (even if pre-print)?
...
We additionally assumed that individuals not in the dataset (approximately 98\%) had
...
% So, just as age-distribution may be different between patches;
% so might the age-specific contacts by contact time
% (for example, a patch may have larger  household sizes; a larger subset of the population who work in high-contact occupations, etc.)
...
\begin{figure}
  ...
  \label{fig:C4AAyi}
  \floatfoot{Contact matrices for Canada, derived from \cite{Prem2017};
