...
% MH: Kind of awkward - would suggest consistency in affirmative vs negative verb tenses
% (here you begin affirmative and switch to negative)
...
\begin{enumerate}
  ...
  the summation of different types, % MH: Already clear you are talking about 1.
  ...
  % MH: A little bit awkward and unclear. I would suggest something like this:
  % "Therefore, when public health restrictions are simulated
  % (e.g. mandated masking, work from home policies etc.)
  % - and overall average contact rates are reduced to reflect reductions in external
  % versus home contacts - theta aa is not updated.
  ...
  % MH: Here I think it's important to specify what methodology underestimates impact of confinement strategies
  While it may be desirable to allow mixing of ``non-mobile'' individuals
...
% MH: Unclear if this is not mobile (as described below), or not mobile outside of home residence patch.
...
% MH: Awkward .... Would recommend as follows:
% "In contrast to [1], we distinguish individuals who are "not mobile" from
% individuals who are mobile within their home residence patch"
As described in \S~\ref{meth.orig}, in \cite{Arenas2020} assume
individuals with reduced mobility form fewer contacts,
but continue mixing with visitors to their residence patch.
In contrast, we categorize mixing into two pools:
``home pools'', where contacts are formed exclusively with
other residents of the same patch $g$ (e.g. household contacts); and
``travel pools'', where contacts are formed with
% MH: redundant
individuals who are mobile within their residence patch
...
Moreover, we assme that the number of contacts formed per person within the home pool is fixed and unaffected by
...
% MH: I would add statement on appendix A.3 here.
\par
