...
\begin{figure}[ht]
  ...
  \caption{Mixing implications for two assumptions about
    ...
    %     so I reference both here now -- does that work? SM: yes
...
% SM: I actually really really liked that you did this [and yes, critical for the hotspot modeling!]. I like that you also put in here as an application (and that you write up everything you do!).
...
\begin{figure}[ht]
  ...
  \begin{subfigure}[t]{0.33\linewidth}
    ...
    \label{fig:phi-box}
  \end{subfigure}
  \caption{Conditional probability of intra-FSA mobility for each decile $\phi_{gt}$  %SM: clarify in footnote for Fig A.4.b that the panel label (1,2, et.c) refer to the home decile.
    versus the population average,
    showing relative stability over time}
  \label{fig:phi}
  \floatfoot{
...
\begin{figure}[ht]
  ...
  \caption{Comparison of element values ($g$, panels; $g'$, colours) in the  %SM: love this figure and cross-check!
    observed mobility matrix $B_{gg't}$, Eq.~(\ref{eq:Bgg.t}) and
    approximated mobility matrix, Eq.~(\ref{eq:Bgg.t.approx}),
